...
contract SoulboundERC1155 is ERC1155LazyMint, SignatureMintERC1155, PrimarySale, PlatformFee  {
    ...
    // Mapping to track if a token is transferable or not
    mapping(uint256 => bool) private _transferableTokens;
    ...
    function mintWithSignature(
        ...
        _transferableTokens[tokenIdToMint] = false;
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public virtual override(ERC1155LazyMint) onlyAllowedOperator(from) {
        require(_transferableTokens[id], "This token is non-transferable");
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(_transferableTokens[ids[i]], "One or more tokens are non-transferable");
        }       
