  ...
  describe("Event Emission", function () {
    describe("Deposited", function () {
      ...
      it("should emit whenever a new deposit is made after creation", async function () {
        ...
        await expect(escrow.connect(funderAccount).deposit(contractId, fakeUSDToken.address, 100)).to.emit(escrow, "Deposited").withArgs(1, funderAccount.address, fakeUSDToken.address, 100);

        await expect(escrow.connect(funderAccount).deposit(contractId, fakeUSDToken.address, 200)).to.emit(escrow, "Deposited").withArgs(1, funderAccount.address, fakeUSDToken.address, 200);

        await expect(escrow.connect(funderAccount).deposit(contractId, fakeUSDToken.address, 300)).to.emit(escrow, "Deposited").withArgs(1, funderAccount.address, fakeUSDToken.address, 300);
      });
    ...
    describe("ContractFinalized", function () {
      it("should emit when the contract is finalized", async function () {
        ...
        await expect(escrow.connect(funderAccount).settle(contractId, false)).to.emit(escrow, "ContractFinalized").withArgs(contractId, funderAccount.address);
      });
    });
  });

  describe("EscrowContract data in storage", function () {
    it("should points same data storage", async function () {
      const { escrow, contractId, funderAccount, payeeAccount } = await prepareEscrowActivation({});

      const expectingState = 1;

      const escrowData = await escrow.getEscrow(contractId);
      expect(escrowData.state).to.equal(expectingState);

      const [result1, _] = await escrow.findEscrowsAsFunderByCursor(funderAccount.address, 0, 1);
      expect(result1[0][1]).to.equal(expectingState);
